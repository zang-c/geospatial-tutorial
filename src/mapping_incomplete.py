...
seaice = seaice.z.rio.write_crs("EPSG:XXXX")
...
gdf = geopandas.GeoDataFrame(geometry=[new], crs="EPSG:XXXX")
...
fram = fram - new
gdf = geopandas.GeoDataFrame(geometry=[fram], crs="EPSG:XXXX")
gdf = gdf.to_crs('EPSG:XXXX')
...
'''
new_shp = geopandas.read_file(new_outfile_path, crs = 'EPSG:XXXX')
fram_shp = geopandas.read_file(fram_outfile_path, crs = 'EPSG:XXXX')
arctic_shp = geopandas.read_file(arctic_outfile_path, crs = 'EPSG:XXXX')
...
'''
fig, ax = plt.subplots()
arctic_ice.mean(dim=[]).plot(marker = '+', 
                                    color = '#000000', 
                                    label = 'Arctic Mean Ice Conc.', 
                                    ax = ax)
polynya_ice.mean(dim=[]).plot(marker = 'o', 
                                      color = "#4FA3BD", 
                                      label = 'Polynya Mean Ice Conc.',
                                      ax = ax)
seaice_fram.mean(dim=[]).plot(marker = 'o', 
                                     color = "#D4624E", 
...
new_polynya_delta = new_polynya_delta.rio.reproject('EPSG:XXXX')
